...
func (input *BeegoInput) Query(key string) string {
	// 首先从URL Params中读取数据，如果没有则尝试从Form中读取数据POST
...
func (input *BeegoInput) CopyBody() []byte {
	...
	// 从Request中读取body, 然后重建一个 ReadCloser
...
func (input *BeegoInput) bind(key string, typ reflect.Type) reflect.Value {
	rv := reflect.Zero(reflect.TypeOf(0)) //? 这个地方似乎存在问题
